...
playlist.get("/", async (request, response) => {
  try {
    response.redirect(`/loginSpotify`);
  } catch (error) {
    response.send(error);
  }
});
...
playlist.get("/loginSpotify", async (request, response) => {
  try {
    response.redirect(`${spotifyLogin}${uri}`);
  } catch (error) {
    response.send(error);
  }
});
...
playlist.get("/playlist", async (request, response) => {
  try {
    ...
    if (request.query.city) {
      ...
      weatherResult = await weather.byLatLonCoordinates(
        request.query.lat,
        request.query.lon
      );
    } else {
    ...
    spotifyResult.songs = await spotify.getTracks(
      songGener,
